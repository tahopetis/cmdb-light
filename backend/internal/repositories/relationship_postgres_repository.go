...
// GetBySourceID retrieves relationships by source ID
func (r *RelationshipPostgresRepository) GetBySourceID(ctx context.Context, sourceID uuid.UUID) ([]*models.Relationship, error) {
	query := `
		SELECT id, source_id, target_id, type, created_at
		FROM relationships
		WHERE source_id = $1
	...
	var relationships []*models.Relationship
	err := r.db.SelectContext(ctx, &relationships, query, sourceID)
	if err != nil {
		return nil, err
	}
...
// GetByTargetID retrieves relationships by target ID
func (r *RelationshipPostgresRepository) GetByTargetID(ctx context.Context, targetID uuid.UUID) ([]*models.Relationship, error) {
	query := `
		SELECT id, source_id, target_id, type, created_at
		FROM relationships
		WHERE target_id = $1
	...
	var relationships []*models.Relationship
	err := r.db.SelectContext(ctx, &relationships, query, targetID)
	if err != nil {
		return nil, err
	}
